...
const AILoadingIndicator = () => (
  <div className="flex flex-col items-center justify-center bg-rich-slate/50 p-6 rounded-xl border border-moonstone/50 mt-6">
    ...
    <p className="text-sm text-neutral-400">
      Crafting your personalized study materials.
    </p>
  </div>
);
...
const CourseDetail = () => {
  ...
  useEffect(() => {
    if (courseId) {
      ...
      const cachedFlashcards = getCachedData<FlashcardType[]>(
        "flashcards",
        courseId
      );
  ...
  const handleGenerate = async (
    ...
    const courseContentText = modules
      ...
      .map(
        (module) =>
          `Module: ${module.name}\n` +
          (module.items
            ?.map((item) => `  - ${item.title} (Type: ${item.type})`)
            .join("\n") || "")
      )
      .join("\n\n");
    ...
    try {
      if (type === "guide") {
        const generatedContent = await generateStudyGuide(
          course.name,
          courseContentText
        );
  ...
  return (
    <div className="animate-fadeIn">
      <Link
        to="/"
        className="flex items-center text-soft-lavender hover:text-violet-300 mb-6 group"
      >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-neutral-50">
            {course?.name || <Skeleton className="h-9 w-80" />}
          </h1>
        ...
        <div className="flex items-center space-x-2 mt-4 md:mt-0">
          ...
          >
            {isGenerating && generationType === "guide" ? (
        ...
        {studyGuideContent && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => handleGenerate("guide", true)}
                disabled={isGenerating}
                className="flex items-center text-sm text-neutral-400 hover:text-soft-lavender disabled:cursor-not-allowed disabled:text-neutral-600"
              >
            ...
            {flashcards.length > 0 ? (
              ...
              <div className="bg-rich-slate/70 border border-moonstone/50 rounded-xl p-8 text-center">
                <p className="text-neutral-300">
                  The AI couldn't identify any key concepts to create flashcards
                  from.
                </p>
              </div>
            )}
      ...
      <div className="mt-8">
        ...
        {modulesError && (
          <ErrorDisplay message="Could not load course modules." />
        )}
        {modules && modules.length === 0 && !isLoading && (
          <div className="bg-rich-slate/70 border border-moonstone/50 rounded-xl p-8 text-center">
            <p className="text-neutral-300">
              This course doesn't seem to have any published modules yet.
            </p>
            <p className="text-sm text-neutral-400 mt-1">
              Check back later when your instructor adds materials!
            </p>
          </div>
        )}
        <div className="space-y-4">
          {modules?.map((module) => (
            <div
              key={module.id}
              className="bg-rich-slate/70 border border-moonstone/50 rounded-xl p-4"
            >
              ...
              {module.items && module.items.length > 0 ? (
                <ul className="mt-2 space-y-2">
                  {module.items.map((item) => (
                    <li
                      key={item.id}
                      className="flex items-center text-neutral-300 pl-4 py-1 border-l-2 border-moonstone"
                    >
                      {item.type === "File" ? (
                        <FileText className="w-4 h-4 mr-3 text-gentle-peach" />
                      ) : (
                        <BookOpen className="w-4 h-4 mr-3 text-fresh-mint" />
                      )}
                      <a
                        href={item.html_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="hover:text-soft-lavender hover:underline"
                      >
                        {item.title}
                      </a>
                    </li>
                ...
                <p className="text-sm text-neutral-400 mt-2 pl-4">
