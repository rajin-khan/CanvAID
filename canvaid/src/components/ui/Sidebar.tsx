...
const NavItem = ({ icon: Icon, text, href, active = false, disabled = false }: { icon: React.ElementType; text: string; href: string; active?: boolean; disabled?: boolean }) => {
  const commonClasses = "flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 group";
  
  if (disabled) {
    return (
      <div className={`${commonClasses} text-neutral-600 cursor-not-allowed`}>
  ...
  return (
    <Link to={href} className={`${commonClasses} ${active ? 'bg-soft-lavender/10 text-neutral-50 shadow-inner shadow-soft-lavender/5' : 'text-neutral-400 hover:bg-rich-slate hover:text-neutral-100'}`}>
  ...
  return (
    <div className="hidden md:flex flex-col w-64 bg-rich-slate/50 backdrop-blur-lg border-r border-moonstone/50">
      ...
      <div className="flex-1 flex flex-col pt-6 px-4 space-y-2">
        <NavItem icon={Home} text="Dashboard" href="/" active={location.pathname === '/'} disabled={!isLoggedIn} />
        <NavItem icon={BookOpen} text="Courses" href="#" active={location.pathname.startsWith('/courses')} disabled={!isLoggedIn} />
        <NavItem icon={CheckSquare} text="Assignments" href="/assignments" active={location.pathname.startsWith('/assignments')} disabled={!isLoggedIn} />
