...
const NavItem = ({ icon: Icon, text, active = false }: { icon: React.ElementType; text: string; active?: boolean }) => (
  <a
    ...
    className={`
      ...
      ${
        active
          ? 'bg-soft-lavender/10 text-neutral-50 shadow-inner shadow-soft-lavender/5'
  ...
  return (
    <div className="hidden md:flex flex-col w-64 bg-rich-slate/50 backdrop-blur-lg border-r border-moonstone/50">
      <div className="flex items-center justify-center h-20 border-b border-moonstone/50">
        {/* New Wordmark Logo with Gradient */}
      ...
      <div className="flex-1 flex flex-col pt-6 px-4 space-y-2">
        ...
        <NavItem icon={CheckSquare} text="Assignments" />
        <div className="flex-grow" />
        <NavItem icon={LifeBuoy} text="Help & Support" />
        ...
        <div className="p-4 mt-4 bg-moonstone/30 rounded-lg text-center border border-moonstone/50">
            ...
            <button className="
              mt-4 text-sm font-semibold text-white bg-linear-to-r from-violet-500 to-pink-500 
              px-4 py-2.5 rounded-lg w-full transition-all duration-300
